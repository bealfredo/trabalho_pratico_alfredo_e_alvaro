// relacionado ao carrossel
const buttonsCarrossel = document.querySelectorAll("#carrossel .controls button")
const divSlides = document.querySelector("#carrossel .slides")

function passSlide(slideNumber) {
    let l = buttonsCarrossel.length
    for(let i=0; i<l; i++) {
        buttonsCarrossel[i].classList.remove("selected")
    }
    divSlides.style.marginLeft = "-" + (slideNumber * 100) + "%";
    buttonsCarrossel[slideNumber].classList.add("selected")
}
